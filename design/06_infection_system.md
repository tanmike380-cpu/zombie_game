...
- Infection itself does not create a separate long-range attraction field. Long-range zombie attraction is handled by the Noise system generated by combat.

Goal:

> A frontline mistake should not only remove one soldier; it can immediately create new enemies and make a local collapse harder to recover from.

## Building Infection Rule

When a building is captured/infected by zombies, it does not release only a one-time burst. It becomes a **Zombie Nest**.
...
A major building collapse or infection event may generate a one-time **destruction Noise**, but the nest does not need a permanent artificial attraction aura in V1.
...
Low-tier zombies spawn immediately
  ↓
The ongoing battle keeps generating Noise
  ↓
Nearby roaming zombies are attracted toward the fighting player army
  ↓
...
- 感染本身不再生成独立的远距离吸引场。远距离吸引统一由战斗产生的 **Noise / 声音系统**负责。

目标：

> 一个前排失误，不只是少一个兵，而是可能立刻制造新的敌人，让局部崩盘更难挽回。
...
大型建筑倒塌/被感染时可以产生一次性的**破坏声音 Noise**，但 V1 不需要让尸巢永久自带一个额外吸引光环。
